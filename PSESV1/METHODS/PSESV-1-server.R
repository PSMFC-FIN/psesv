...
shinyServer(
  function(input,output) {
    ...
    plotdist<-reactive({
      ...
      data1  <- length[SURVEY_DEFINITION_ID==survey & SPECIES_CODE==species]
    ...
    tempplot<-reactive({
      survey  <- input$survey
      species <- input$species
      location      <- location[SURVEY_DEFINITION_ID==survey]
      length        <- length[SURVEY_DEFINITION_ID==survey & SPECIES_CODE==species]
    ...
    plotx<-reactive({    
      ...
      location      <- location[SURVEY_DEFINITION_ID==survey]
      length        <- length[SURVEY_DEFINITION_ID==survey & SPECIES_CODE==species]
      location_poll <- location_poll[SURVEY_DEFINITION_ID==survey & SPECIES_CODE==species]
      SN            <- SN[SPECIES_CODE==species]
