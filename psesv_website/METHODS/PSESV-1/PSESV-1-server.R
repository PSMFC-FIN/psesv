...
shinyServer(
  ...
  output$plot1 <- renderPlot({
    ...
    withProgress(message = 'Making plot', value = 0, {   
     ...
     if(input$AREA_W.cent==T & input$survey.cent %in% c(46,47,48) )
	      {   survey.cent<-input$survey.cent
            area <- SURVEY_AREA(location=location,surveyx=survey.cent) 
  ...
  output$plot3 <-renderPlot({
      data1  <- length[SURVEY_DEFINITION_ID==input$survey.len & SPECIES_CODE==input$species.len]
  ...
  output$plot4 <- renderPlot({
    withProgress(message = 'Making plot', value = 0, {
       ...
       location      <- location[SURVEY_DEFINITION_ID==input$survey.dis]
       length        <- length[SURVEY_DEFINITION_ID==input$survey.dis & SPECIES_CODE==input$species.dis]
       location_poll <- location_poll[SURVEY_DEFINITION_ID==input$survey.dis & SPECIES_CODE==input$species.dis]
       SN            <- SN[SPECIES_CODE==input$species.dis]
       ...
       if(input$survey.dis %in% c(46,47,48)){
       ...
       if(input$survey.dis==98 & !1994 %in% length$YEAR){ location<-location[YEAR %in% yers]}
       if(input$survey.dis %in% c(46,47,48) & !1993 %in% length$YEAR){ location<-location[YEAR %in% yers]}
       if(input$survey.dis==52 & !1994 %in% length$YEAR){ location<-location[YEAR %in% yers]}
       ...
       ## Temperature plot
       if(input$plotT.dis%in%c(1,3,5,7,9))
       { 
         incProgress(1/5)
       ...
       ## Location plot 
       if(input$plotT.dis%in% c(2,4,6,8))
       { 
       ...
       if(input$plotT.dis%in%c(1,3,5,7,9)){ dataT <- merge(location1,dataT,all.x=T,by=c("YEAR","TEMPR","DEPTHR"),allow.cartesian=TRUE)}
       if(input$plotT.dis%in%c(2,4,6,8)){ dataT <- merge(location1,dataT,all.x=T,by=c("YEAR","LON","LAT"),allow.cartesian=TRUE)}
       ...
       if(input$plotT.dis%in%c(2,4,6,8))
       {
         ...
         if(input$survey.dis[1]==98){  p <- p + coord_fixed(ylim=c(51,65),xlim=c(179,205))
         nc=5}
         if(input$survey.dis[1]==46){  p <- p + coord_fixed(ylim=c(50,58),xlim=c(190,201))
         nc=3}
         if(input$survey.dis[1]==47){  p <- p + coord_fixed(ylim=c(52,61.5),xlim=c(200,213))
         nc=3}
         if(input$survey.dis[1]==48){  p <- p + coord_fixed(ylim=c(53,61.5),xlim=c(213,230))
         nc=3}
         if(input$survey.dis[1]==52){  p <- p + coord_fixed(ylim=c(51,55),xlim=c(170,195))
         nc=2}
         if(input$survey.dis[1]==78){  p <- p + coord_fixed(ylim=c(51,63),xlim=c(175,195))
         nc=2}
         ...
         if(input$survey.dis[1]==98){  d <- d + scale_x_continuous(breaks=seq(180,205,by=5),labels=seq(180,155,by=-5)) + ylab(expression(paste("Latitude ",degree," N",sep="")))+xlab(expression(paste("Longitude ",degree," W",sep=""))) }
         if(input$survey.dis[1]==46){  d <- d + scale_x_continuous(breaks=seq(190,216,by=2),labels=seq(170,144,by=-2)) + ylab(expression(paste("Latitude ",degree," N",sep="")))+xlab(expression(paste("Longitude ",degree," W",sep=""))) }
         if(input$survey.dis[1]==47){  d <- d + scale_x_continuous(breaks=seq(190,216,by=2),labels=seq(170,144,by=-2)) + ylab(expression(paste("Latitude ",degree," N",sep="")))+xlab(expression(paste("Longitude ",degree," W",sep=""))) }
         if(input$survey.dis[1]==48){  d <- d + scale_x_continuous(breaks=seq(190,230,by=2),labels=seq(170,130,by=-2)) + ylab(expression(paste("Latitude ",degree," N",sep="")))+xlab(expression(paste("Longitude ",degree," W",sep=""))) }
         
         
         if(input$survey.dis[1]==52){  d <- d + scale_x_continuous(breaks=seq(170,195,by=5),labels=c(170,175,seq(180,165,by=-5))) +ylab(expression(paste("Latitude ",degree," N",sep="")))+xlab(expression(paste("Longitude",sep=""))) }
         if(input$survey.dis[1]==78){  d <- d + scale_x_continuous(breaks=seq(175,195,by=5),labels=c(175,180,175,170,165)) +ylab(expression(paste("Latitude ",degree,sep="")))+xlab(expression(paste("Longitude ",degree," W",sep=""))) }
  ...
  output$plot5 <- renderPlot({
    withProgress(message = 'Making plot', value = 0, {
      ...
      plot_BIOM(data=biomass,ptype="B")
      })
    })
  ...
  output$plot6 <- renderPlot({
      withProgress(message = 'Making plot', value = 0, {
        ...
        plot_BIOM(data=biomass,ptype="P")
